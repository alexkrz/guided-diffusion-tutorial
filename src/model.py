...
class NormActConv(nn.Module):
    ...
    def __getitem__(self, index):
        if index == 0:
            ...
            return nn.Sequential(self.act, self.conv)
        else:
    ...
    def __init__(
        ...
        self.cv = (
            nn.Sequential(
                ...
                nn.Conv2d(
                    in_channels, out_channels // 2 if use_mpool else out_channels, kernel_size=4, stride=k, padding=1
                ),
            )
            if use_conv
            else nn.Identity()
        ...
        self.mpool = (
            nn.Sequential(
                nn.MaxPool2d(k, k),
                nn.Conv2d(
                    in_channels, out_channels // 2 if use_conv else out_channels, kernel_size=1, stride=1, padding=0
                ),
            )
            if use_mpool
            else nn.Identity()
...
class DownC(nn.Module):
    ...
    def __init__(
        ...
        self.te_block = nn.ModuleList(
            [
                TimeEmbedding(2 * out_channels if use_scale_shift_norm else out_channels, t_emb_dim)
...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        self.cv2 = nn.Sequential(
            nn.GroupNorm(8, self.up_ch[-1]), nn.Conv2d(self.up_ch[-1], self.im_channels * 2, kernel_size=3, padding=1)
        )
